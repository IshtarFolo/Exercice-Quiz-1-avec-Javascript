...
function recupererHistorique() {
    ...
    let historiqueChaine = localStorage.getItem("historiqueQuiz");
    // On retourne l'historique décodé en JS ou un tableau vide
    return JSON.parse(historiqueChaine) || []; //Transforme en chaine et ramene en haut avec return, IMPORTANT 
}
...
function commencerLeQuiz() {
    ...
    historique.push(
        {date: new Date().toLocaleDateString('fr-CA'), reponses: []} //new Date().toLocaleDateString('fr-CA') --> Spécification date culture ou lieux, exemple ici canada francais ou encore EN-US, etc
    );
        
    // Modifier la valeur stockée dans localStorage
    localStorage.setItem("historiqueQuiz", JSON.stringify(historique)); //IMPORTANT JSON.stringify!!!!
...
function verifierReponse(event) {
    ...
    // historique.
    historique[historique.length-1].reponses.push(event.target.indexChoix); //La longeur du tableau -1 pour donner la dernière position .reponse pour accéder aux réponses et .push l'élément cliqué par joueur
...
function afficherFinQuiz() {
    ...
    //historique;
    let nombreParties = historique.length; //Donne index tableau AKA nbr entrées dans tableau
    ...
    for (let partie of historique) {
        zoneFinQuiz.innerHTML += "<p>"; //item de liste
